...
const btnBase = 'px-4 py-2 rounded-lg text-[13px] font-medium cursor-pointer border-none transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96]';
  ...
  return (
    <div className="options max-w-[800px] mx-auto">
      <header className="mb-6 pb-4 border-b border-border">
        <h1 className="text-xl font-semibold text-text mb-1">中文校对助手 - 设置</h1>
        <p className="text-muted text-sm">行业专业词库管理</p>
      </header>

      {message && (
        <div className={cn('toast fixed top-5 right-5 px-4 py-3 rounded-lg text-sm font-medium z-[9999]', message.type === 'success' ? 'bg-success-bg text-success border border-success-border' : 'bg-error-bg text-error border border-error-border')}>
          {message.text}
        </div>
      )}
      ...
      <div className="bg-white border border-border rounded-xl p-5 mb-5">
        ...
        <div className="flex items-center gap-3 mb-3.5">
          <label className="w-20 text-[13px] text-muted font-medium shrink-0">动作</label>
          <select
        ...
        {newEntry.action === 'correct' && (
          <div className="flex items-center gap-3 mb-3.5">
            <label className="w-20 text-[13px] text-muted font-medium shrink-0">纠正为</label>
            <input
              type="text"
        ...
        <div className="flex items-center gap-3 mb-3.5">
          <label className="w-20 text-[13px] text-muted font-medium shrink-0">所属域</label>
          <select
            multiple
      ...
      <div className="list-card bg-white border border-border rounded-xl p-5">
        <h2 className="text-[15px] font-semibold text-text mb-4">词条列表 ({filteredEntries().length})</h2>
            ...
            <tbody>
              {filteredEntries().map((entry, i) => (
                <tr key={i} className="hover:bg-surface">
                  ...
                  <td>
                    <span className={cn('inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold', entry.action === 'ignore' ? 'bg-error-bg text-error' : 'bg-success-bg text-success')}>
                  ...
                  <td>
                    {entry.domains?.map(d => (
                      <span key={d} className="inline-block px-1.5 py-0.5 mr-1 rounded bg-[#e5e9f0] text-muted text-[11px]">{d}</span>
                    )).join(' ') || '—'}
                  </td>
                  <td>
      ...
      <footer className="mt-6 pt-4 border-t border-border text-center">
        <p className="text-[#94a3b8] text-xs">词典修改实时生效，无需重启扩展。数据存储在浏览器本地，不上传云端。</p>
      </footer>
    </div>
  );
}
