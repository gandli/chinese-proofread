...
export default function App() {
  ...
  const actionBase =
    "w-full px-3.5 py-[11px] text-sm font-medium text-white bg-text rounded-[9px] cursor-pointer text-left flex items-center gap-2 transition-colors duration-150 mb-1.5 hover:bg-[#2a3650] active:scale-[0.96] disabled:opacity-50 disabled:cursor-default focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent";
  ...
  return (
    <div className="app w-[380px] p-4">
      <header className="header flex items-center gap-2.5 mb-3.5">
        ...
        <div className="header-text">
          ...
          <span className="badge flex items-center gap-[5px] mt-0.5 text-[11px] text-muted">
            <span className="dot w-1.5 h-1.5 rounded-full bg-success inline-block" />
            本地引擎 · 离线可用
          </span>
        </div>
      </header>
      ...
      >
        {isBusy && (
          <span className="spinner inline-block w-[13px] h-[13px] border-2 border-white/35 border-t-white rounded-full align-[-2px] mr-[7px] animate-[spin_0.8s_linear_infinite]" />
        )}
      ...
      {errMsg && (
        <div className="error text-[12.5px] text-error bg-error-bg border border-error-border rounded-lg px-[11px] py-2.5 my-1.5 leading-normal">
          {errMsg}
        </div>
      )}
      ...
      {status === "done" && stats && (
        <p
          className={cn(
            "status text-[12.5px] text-muted py-0.5 pb-2",
        ...
        >
          ...
          <span>
            {" "}
            · {stats.chars} 字 · {(stats.timeMs / 1000).toFixed(1)}s
          </span>
        </p>
      )}
