...
interface TestHook {
  setState: (s: Partial<SidePanelState>) => void;
}
  ...
  return (
    <div className="diff-item bg-surface border border-border rounded-[10px] p-3 transition-[box-shadow,border-color] duration-150 hover:shadow-[0_4px_16px_rgba(15,23,42,0.08)] hover:border-border-subtle">
      <div className="diff-header flex items-center justify-between mb-1.5">
        <span className="diff-index text-xs font-semibold text-muted">
          #{index + 1}
        </span>
      ...
      <div className="diff-actions flex gap-2">
        <button
          className="btn px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer border-none transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96] bg-success text-white hover:bg-[#065f46]"
          onClick={() => onAccept(diff)}
        >
          采用
        </button>
        <button
          className="btn px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer border transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96] bg-white text-text-secondary border-border-subtle hover:bg-background-subtle hover:border-[#94a3b8]"
          onClick={() => onIgnore(diff)}
        >
          忽略
        </button>
        <button
          className="btn px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer border-none transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96] bg-primary text-white hover:bg-primary-hover"
          onClick={() => onJump(diff)}
        >
          跳转
        </button>
...
function SidePanel() {
  ...
  useEffect(() => {
    const testHook: TestHook = {
      setState: (s) => setState((prev) => ({ ...prev, ...s })),
    };
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(
      ["ps-last-diffs", "ps-last-text", "ps-active-tab"],
      (res) => {
    ...
    <div className="panel h-screen flex flex-col bg-white overflow-hidden">
      <header className="panel-header flex items-center justify-between px-4 py-3.5 border-b border-border bg-surface shrink-0">
        <h1 className="text-[15px] font-semibold text-text">
          校对结果{" "}
          <span className="count text-xs font-normal text-error bg-error-bg px-2 py-0.5 rounded-full ml-2">
            {state.diffs.length} 处
          </span>
        </h1>
        <div className="batch-actions flex gap-2">
          <button
            className="btn px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer border-none transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96] bg-[#e5e9f0] text-text hover:bg-border-subtle"
            onClick={handleAcceptAll}
          >
            全部采用
          </button>
          <button
            className="btn px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer border-none transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:scale-[0.96] bg-error-bg text-error hover:bg-[#fecaca]"
