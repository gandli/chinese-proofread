...
import { loadCustomDictNode, applyCustomDictNode } from '../src/utils/custom-dict.node';
...
const chunkDiffs = await Promise.all(chunks.map(async (ch) => (await c.correct(ch.text)).diffs));
const diffs = mergeDiffs(chunks, chunkDiffs);

console.log(`原文 ${text.length} 字 | 模型检出 ${diffs.length} 处:`);
for (const d of diffs) {
  console.log(`  #${d.position} "${d.original}" → "${d.corrected}" (${(d.confidence * 100).toFixed(0)}%)`);
}
