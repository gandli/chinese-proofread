...
export class MacBertCorrector {
  ...
  async correct(text: string): Promise<CorrectionResult> {
    ...
    if (text.length > MAX_LEN - 2)
      throw new Error(`Text too long (>${MAX_LEN - 2} chars), split first`);
    ...
    const feeds = {
      ...
      attention_mask: new ort.Tensor("int64", pad(ids.map(() => 1)), [
        1,
        MAX_LEN,
      ]),
      token_type_ids: new ort.Tensor("int64", new BigInt64Array(MAX_LEN), [
        1,
        MAX_LEN,
      ]),
    };
