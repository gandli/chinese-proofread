...
describe("splitLongText", () => {
  ...
  it("超长文本按句分段", () => {
    ...
    expect(total.length).toBeGreaterThanOrEqual(
      text.length - 20 * chunks.length,
    );
  });
  ...
  it("参数校验：maxChars 必须 > 0", () => {
    expect(() => splitLongText("测试", 0)).toThrow("maxChars must be > 0");
    expect(() => splitLongText("测试", -1)).toThrow("maxChars must be > 0");
  });
  ...
  it("不产生空 chunk", () => {
    const text = "。".repeat(10); // 纯标点
    const chunks = splitLongText(text, 5, 1);
    for (const c of chunks) {
  ...
  it("offset 边界校验", () => {
    const text = "测试文本";
    const chunks = splitLongText(text, 3, 0);
    for (const c of chunks) {
...
describe("mergeDiffs", () => {
  ...
  it("chunks 与 chunkDiffs 长度不匹配抛错", () => {
    const chunks = [{ text: "测试", offset: 0 }];
    const chunkDiffs = [[], []];
    expect(() => mergeDiffs(chunks, chunkDiffs)).toThrow("length mismatch");
  });

  it("越界 diff 被静默丢弃", () => {
    const chunks = [{ text: "AB", offset: 0 }];
    const chunkDiffs = [
      [
        { original: "A", corrected: "X", position: 5, confidence: 0.9 }, // 越界
        { original: "B", corrected: "Y", position: 1, confidence: 0.9 },
      ],
    ];
    const merged = mergeDiffs(chunks, chunkDiffs);
    expect(merged).toHaveLength(1);
  ...
  it("负 position diff 被静默丢弃", () => {
    const chunks = [{ text: "AB", offset: 0 }];
    const chunkDiffs = [
      [
        { original: "A", corrected: "X", position: -1, confidence: 0.9 }, // 越界
        { original: "B", corrected: "Y", position: 1, confidence: 0.9 },
      ],
    ];
    const merged = mergeDiffs(chunks, chunkDiffs);
